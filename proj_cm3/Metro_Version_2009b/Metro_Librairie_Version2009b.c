...
#define __ALTERNATE_OUTPUT_PUSH_PULL_PORTA           0x003000B0
...
#define __CC4_OFF									 0xEFFF
#define __CC4_ON									 0x1000
...
GPIOA->CRL    =__ALTERNATE_OUTPUT_PUSH_PULL_PORTA ; 	              // port pA1 config as alternate function because it's output pwm(TIM2_CH2), and pA5 config output push pull
...
{ TIM2->CR1 &=__TIMX_CR1_CDISABLE ;
  TIM2->CCER &=__CC2_OFF;
  TIM4->CCER |=__CC4_ON;					                  // output control oc4 signal is output on the corresponding output pin
  TIM4->CR1  |= __TIMX_CR1_CEN;
}
...
{ TIM4->CR1 &=__TIMX_CR1_CDISABLE;
  TIM4->CCER &=__CC4_OFF;
  TIM2->CCER |=__CC2_ON; 		                                // output control oc2 signal is output on the corresponding output pin
