...
#include <stdlib.h>
					   ...
					   /*----mettez 1 ou 0 pour activer et désactiver ces fonctionalites----*/
#define use_usart3   1
#define use_capteur_vitesse 1
#define use_adc  1
#define use_capteur_incrementale 1
#define use_output_pwm 1
				    /*--------------------------------------end---------------------------------*/

#define dt 0.001     // pas de calcule de la vitesse
...
{ 
 static int position =0;
  int p;
  p=Lire_Position();
  vitesse=(p-position)/(dt);	 
  position =p;
 }

 short int Lire_Vitesse()
{
   return vitesse;
}
...
void Init_Periphs()			  
{
 ...
 #if use_capteur_incrementale
 Init_Timer3();
 #endif

 #if use_capteur_vitesse
 Init_Timer4();
 #endif
